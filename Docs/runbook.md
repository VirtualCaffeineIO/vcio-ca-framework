...
* Transition exit: where `SG-CA-Transition-Hybrid` is in use, schedule `Tools/Invoke-VcioTransitionExit.ps1 -Manifest <path> -Confirm:$false` daily (Azure Automation or the MSP's job host). **`-Confirm:$false` is required for an unattended run** — the cmdlet is `ConfirmImpact='High'` and will otherwise block on the first removal prompt, which looks identical to a job with nothing to do. Where Entra ID Governance is licensed, a recurring access review with auto-remove on the group is the supported alternative. Expiry is monitored by a shipped script against an operator deadline — it is not enforced by the policy objects, which have nowhere to carry a date.
